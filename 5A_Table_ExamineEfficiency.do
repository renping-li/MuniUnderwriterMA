...
label var treatedXpostXbank_not_involved "Treated $\times$ Post $\times$ Bank is not in M\&A"
...
reghdfe has_rating treatedXpost ///
...
if year_to_merger>=-4&year_to_merger<=4, ///
absorb(i.issuer_code##i.issuer_type##i.episode_start_year##i.treated_csa i.episode_start_year##i.treated_csa##i.calendar_year) ///
cluster(issuer_code) noconstant
...
if year_to_merger>=-4&year_to_merger<=4, ///
absorb(i.issuer_code##i.issuer_type##i.episode_start_year##i.treated_csa i.episode_start_year##i.treated_csa##i.calendar_year) ///
cluster(issuer_code calendar_year) noconstant
...
label var treatedXpostXif_insured "Treated $\times$ Post $\times$ If Insured"
label var treatedXpostXif_not_insured "Treated $\times$ Post $\times$ If Not Insured"
...
outreg2 using  "`outfile'", tex(fragment) append label ///
keep(treatedXpostXif_insured treatedXpostXif_not_insured) `outputoptions' ctitle("Has","Rating") ///
addstat("Adjusted R-squared", e(r2_a)) addtext("Issuer $\times$ Cohort FE", "Yes","Cohort $\times$ Year FE", "Yes","Clustering","CSA \& Year") ///
...
// Column 4: Having insurance
reghdfe insured_ratio treatedXpost ///
if year_to_merger>=-4&year_to_merger<=4, ///
absorb(i.issuer_code##i.issuer_type##i.episode_start_year##i.treated_csa i.episode_start_year##i.treated_csa##i.calendar_year) ///
cluster(csacode calendar_year) noconstant
...
// Column 5: Having insurance, with control variables
...
local effects_insured_ratio_inv = _b[treatedXpostXbank_is_involved]*100
local effects_insured_ratio_inv : display %-9.1f `effects_insured_ratio_inv'
file open myfile using "../Draft/nums/effects_insured_ratio_inv.tex", write replace
file write myfile "`effects_insured_ratio_inv'"
file close myfile

local t_effects_insured_ratio_inv = _b[treatedXpostXbank_is_involved]/_se[treatedXpostXbank_is_involved]
local t_effects_insured_ratio_inv : display %-9.1f `t_effects_insured_ratio_inv'
file open myfile using "../Draft/nums/t_effects_insured_ratio_inv.tex", write replace
file write myfile "`t_effects_insured_ratio_inv'"
file close myfile
...
// Column 6: Having an advisor
reghdfe if_advisor_coded treatedXpost ///
if year_to_merger>=-4&year_to_merger<=4, ///
absorb(i.issuer_code##i.issuer_type##i.episode_start_year##i.treated_csa i.episode_start_year##i.treated_csa##i.calendar_year) ///
cluster(csacode calendar_year) noconstant
...
// Column 7: Having an advisor, with control variables
...
addstat("Adjusted R-squared", e(r2_a)) addtext("Issuer $\times$ Cohort FE", "Yes","Cohort $\times$ Year FE", "Yes","Clustering","CSA \& Year") ///
sortvar(treatedXpost treatedXpostXbank_not_involved treatedXpostXbank_is_involved treatedXpostXif_insured treatedXpostXif_not_insured)
...
local outfile =  "../Draft/tabs/Slides_DID_MAEfficiency_ByIfMerging.tex"
local outputoptions = "nor2 dec(2) stats(coef tstat) nocons tdec(2) nonotes adec(3)"
...
gen has_rating = (has_fitch=="True")|(has_moodys=="True")
replace has_rating = has_rating*100
...
replace insured_amount = 0 if insured_amount==.
gen insured_ratio = insured_amount/amount
replace insured_ratio = insured_ratio*100

// Column 2: Having insurance, with control variables
reghdfe insured_ratio (i.treated i.post)##(i.bank_is_involved) treatedXpostXbank_not_involved treatedXpostXbank_is_involved ///
if year_to_merger>=-4&year_to_merger<=4, ///
absorb(i.issuer_code##i.issuer_type##i.episode_start_year##i.treated_csa i.episode_start_year##i.treated_csa##i.calendar_year) ///
cluster(csacode calendar_year) noconstant
...
gen if_advisor_coded = if_advisor=="Yes"
replace if_advisor_coded = if_advisor_coded*100
